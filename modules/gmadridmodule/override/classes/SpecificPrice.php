...
class SpecificPrice extends SpecificPriceCore
{
                                            ...
                                            $real_quantity = 0)
    {
        ...
        */
        ...
        if (!static::couldHaveSpecificPrice($id_product)) {
            return array();
        }
        ...
        if (!array_key_exists($key, static::$_specificPriceCache)) {
            if($id_customer) {
                $queryGroups = 'SELECT id_group FROM `'._DB_PREFIX_.'customer_group`'.' WHERE id_customer = '.$id_customer;
                $id_groups = Db::getInstance(_PS_USE_SQL_SLAVE_)->query($queryGroups)->fetchAll();
                $id_groups = array_map(function($item) { return $item["id_group"];}, $id_groups);
            } else {
                $id_groups = array();
            }

            $query_extra = static::computeExtraConditions($id_product, $id_product_attribute, $id_customer, $id_cart);
			...
			SELECT *, '.SpecificPrice::_getScoreQuery($id_product, $id_shop, $id_currency, $id_country, $id_group, $id_customer).'
				FROM `'._DB_PREFIX_.'specific_price`
				WHERE
                `id_shop` '.static::formatIntInQuery(0, $id_shop).' AND
                `id_currency` '.static::formatIntInQuery(0, $id_currency).' AND
                `id_country` '.static::formatIntInQuery(0, $id_country).' AND
                `id_group` '.static::formatIntsInQuery(0, $id_groups).' '.$query_extra.'
    ...
    protected static function formatIntsInQuery($first_value, array $values)
    {
        $first_value = (int)$first_value;
        if (count($values)>0) {
            return 'IN ('.$first_value.', '.implode($values, ',').')';
        } else {
            return ' = '.$first_value;
        }
    }
